...
sobre un conjunto no vac\'\i o $X$ (que eventualmente puede tener
m\'as estructura si conviene). De este modo el espacio de Hilbert $H$
que consideramos aqu\'\i\ es un subespacio de
$\mathfrak{F}(X,\mathbb{R})=\mathbb{R}^X$:
$H\subset\mathfrak{F}(X,\mathbb{R})=\mathbb{R}^X$.

\textbf{Nota:}  $\mathfrak{F}(X,\mathbb{R})$ denota el conjunto de
funciones que van de $X$ a $\mathbb{R}$.
...
\begin{enumerate}[(a)]
\item Los funcionales sobre un espacio de
Hilbert, ?`son todos lineales y continuos?
...
\item ¿Son continuos todos los funcionales de evaluación sobre un espacio de Hilbert de funciones sobre un conjunto no vacío dado?
