...
\newpage
Los \textbf{funcionales de evaluación} sobre un espacio de Hilbert
\textbf{no son siempre continuos}. Aquí consideramos un conjunto no
vacío $X$, un espacio de Hilbert $H$ de funciones reales sobre $X$, es
decir, $H\subset\mathfrak{F}(X,\mathbb{R})=\mathbb{R}^X$, y fijamos un
elemento genérico $x\in X$. Por lo tanto, la condici\'on
(\ref{stetigkeit}) no siempre se cumple, como lo demuestra el
...
$\mathcal{H}$. {\bf (!`Demu\'estrelo!)}

Para que la relación ``$\sim$'' sea una relación de
equivalencia debe cumplir lo siguiente:

\begin{description}
\item[Reflexividad] todo elemento de $\mathcal{H}$ debe estar
relacionado consigo mismo, es decir:
\begin{equation*}
\forall f \in \mathcal{H}: f \sim f
\end{equation*}

se tiene entonces que:

\begin{equation*}
f \sim f \quad\Leftrightarrow\quad
\int_{S(f,f)} |f(x)-f(x)|^2\,dx= \int_{S(f,f)} |0|^2\,dx=0\,,\qquad f\in\mathcal{H}\,,
\end{equation*}

\item[Simetría] se debe cumplir que:
\begin{equation*}
\forall f,g \in \mathcal{H}: f \sim g \Rightarrow g \sim f
\end{equation*}

dado que $f,g$ están definidas en $\mathbb{R}$, $|\cdot|$ denota el
valor absoluto usual:

\begin{equation*}
f \sim g \Leftrightarrow
\int_{S(f,g)} |f(x)-g(x)|^2\,dx=\int_{S(g,f)} |g(x)-f(x)|^2\,dx=0 
\Leftrightarrow g \sim f 
%\,,\qquad
%f,g\in\mathcal{H}\,,
\end{equation*}

\item[Transitividad] se debe cumplir que:

\begin{equation*}
\forall f,g,z \in \mathcal{H}:\quad f \sim z \quad\wedge\quad z \sim g
\Rightarrow f \sim g
\end{equation*}
se tiene que:
